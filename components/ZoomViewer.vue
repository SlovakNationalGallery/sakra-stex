...
const { tileSources } = defineProps(["tileSources"]);
...
<template>
  <div ref="target" class="absolute w-screen h-screen"></div>
</template>
<!-- <template>
...
export default {
  ...
  computed: {
    thumbnailUrls() {
      return this.tileSources.map((ts) =>
        ts.replace(/\.dzi$/, "_files/0/0_0.jpg")
      );
    },
    sequenceMode() {
